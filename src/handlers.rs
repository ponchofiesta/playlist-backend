...
pub async fn get_plays(
    ...
    let connection = pool.get().map_err(|e| {
        HttpResponse::InternalServerError().json(Error::new(&format!("Database connection failed: {}", e)))
    })?;
    ...
    let plays = db::get_full_plays(&connection, &station, &date).map_err(|e| {
        HttpResponse::InternalServerError().json(Error::new(&format!("Could not load plays: {}", e)))
    })?;
